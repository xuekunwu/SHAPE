...
SHAPE supports iterative multi-turn dialogue to handle complex image analysis queries that require sequential reasoning and adaptive tool selection. The conversational planning mechanism is implemented through a stateful `Solver` class (`app.py`, lines 1233-2361) that maintains conversation context across multiple analysis steps.

**Conversation History Management**: The system maintains a persistent conversation history stored in `agent_state.conversation` (list of `ChatMessage` objects). At each planning step, the conversation history is formatted into a plain-text transcript using the `_format_conversation_history()` method (lines 1269-1277), which serializes all previous user queries and system responses into a structured text format:
...
**Context Propagation**: The formatted conversation context (`conversation_text`) is propagated to all planning stages: (1) initial query analysis via `planner.analyze_query()` (line 1568), which determines required tools and skills; (2) step-by-step tool selection via `planner.generate_next_step()` (line 1671), which selects the next tool based on current state and conversation history; (3) completion verification via `planner.verificate_memory()` (line 2094); and (4) final output generation via `planner.generate_final_output()` (line 2272). This ensures that each planning decision incorporates the full dialogue context, enabling the system to handle follow-up questions, refine previous analyses, and adapt to user feedback.

**Iterative Execution Loop**: The execution loop (`app.py`, lines 1656-2118) implements a maximum of `max_steps` (default: 10) iterations. At each step `i`, the planner receives the conversation context (formatted via `_format_conversation_history()`, line 1669), current memory state (containing results from previous tool executions stored in `self.memory`), and the original user query. The planner generates a `NextStep` object via `planner.generate_next_step()` (line 1671) containing the selected tool name, sub-goal, and context. After tool execution via `executor.execute_tool()` (line 1799), results are stored in memory using `memory.add_action()` and the conversation is updated with the tool's output through `messages.append(ChatMessage(...))` (line 1986). This iterative process continues until either the verification step (`planner.verificate_memory()`, line 2094) signals completion (`stop_signal='STOP'`) or the maximum step count is reached. The loop also includes loop detection mechanisms (lines 1687-1721) to prevent infinite execution when the same tool is selected consecutively three or more times.
...
**Output Aggregation**: Visual outputs are collected into `visual_outputs_for_gradio` (list of `(PIL.Image, label)` tuples, initialized at line 1257) for interactive display in the Gradio interface. The `_collect_visual_outputs()` function (lines 949-1230) processes tool execution results and extracts visual files from multiple dictionary keys: `deliverables` (preferred), `visual_outputs`, `overlay_path`, `output_path`, and `mask_path`. Structured data files (`.h5ad` AnnData files, `.zip` archives containing crops) are added to `downloadable_files` (line 1989) for batch download. For multi-image analyses, the system automatically creates unified ZIP archives via `_create_unified_crops_zip()` (lines 750-885) and `_create_unified_masks_zip()` (lines 888-946), organizing crops and masks by group and image ID in a hierarchical directory structure within the ZIP file.

### GPU-Compatible Deep Learning Tool Modules

SHAPE integrates state-of-the-art deep learning models through GPU-compatible tool modules that leverage Hugging Face Spaces' GPU infrastructure. The tool modules act as lightweight wrappers that load, configure, and execute pre-trained models without modifying their core architectures, enabling seamless integration of the latest advances in computer vision (e.g., Vision Transformers, Cellpose-SAM) into the agentic framework.
