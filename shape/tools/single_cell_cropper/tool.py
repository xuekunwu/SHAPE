...
import json
import zipfile
from shape.models.utils import VisualizationConfig
from shape.models.task_state import CellCrop
import tifffile
from shape.models.image_data import ImageData
...
class Single_Cell_Cropper_Tool(BaseTool):
        ...
        try:
            ...
            def save_error_metadata(error_type, error_message):
                ...
                try:
                    ...
                    print(f"Single_Cell_Cropper_Tool: âœ?Error metadata file saved: {metadata_path}")
                    return metadata_path
                except Exception as save_error:
                    print(f"Single_Cell_Cropper_Tool: â?Failed to save error metadata: {save_error}")
                    return None
            ...
            if not os.path.exists(nuclei_mask):
                print(f"âš ï¸ WARNING: Mask file does not exist at: {nuclei_mask}")
                ...
                if query_cache_dir and os.path.isabs(query_cache_dir):
                    ...
                    if os.path.exists(alt_mask_path):
                        print(f"âœ?Found mask at alternative path: {alt_mask_path}")
                        nuclei_mask = alt_mask_path
                    else:
                        print(f"â?Alternative path also does not exist: {alt_mask_path}")
            ...
            if os.path.exists(metadata_path):
                print(f"Single_Cell_Cropper_Tool: âœ?Metadata file saved successfully: {metadata_path}")
            else:
                print(f"Single_Cell_Cropper_Tool: â?ERROR: Metadata file was not saved: {metadata_path}")
            ...
            if total_detected > 0:
                if total_filtered > 0:
                    ...
                    summary_parts.append(f"({total_detected} detected â†?{stats['final_cell_count']} valid crops)")
                else:
            ...
            if cell_crops and len(cell_crops) > 0:
                try:
                    ...
                    if os.path.exists(crops_zip_path) and os.path.getsize(crops_zip_path) > 0:
                        ...
                        print(f"âœ?Created crops zip archive: {crops_zip_path} ({os.path.getsize(crops_zip_path)} bytes)")
                    else:
                        print(f"âš ï¸ Warning: Failed to create crops zip archive")
                except Exception as zip_error:
                    print(f"âš ï¸ Warning: Failed to create crops zip archive: {zip_error}")
                    import traceback
                    traceback.print_exc()
    ...
    def _generate_cell_crops(self, original_img, mask, original_image_path, source_image_id, group, min_area, margin, output_dir, output_format, is_multi_channel=False):
        ...
        Unified logic for all image types:
        - Single-channel images: saved as single-channel (grayscale) in specified format
        - Multi-channel images: saved as multi-channel TIFF to preserve all channel information
        ...
        Args:
            ...
            output_format: Output image format (for single-channel; multi-channel always uses TIFF)
        ...
        for idx, region in enumerate(cell_properties):
            ...
            # - Single-channel: (H, W, 1) when using ImageData
            # - Multi-channel: (H, W, C) where C > 1
            # Fallback loading may produce (H, W) which is converted to (H, W, 1) at line 172-173
            # So original_img should always be 3D at this point, but handle both cases for safety
            if original_img.ndim == 3:
                cell_crop = original_img[new_minr:new_maxr, new_minc:new_maxc, :]  # Preserve all channels
            elif original_img.ndim == 2:
                # Fallback: 2D grayscale (should have been converted earlier, but handle for safety)
                cell_crop = original_img[new_minr:new_maxr, new_minc:new_maxc]
                cell_crop = cell_crop[:, :, np.newaxis]  # Add channel dimension -> (H, W, 1)
            else:
                raise ValueError(f"Unexpected image dimensions: {original_img.ndim}, expected 2 or 3")
            ...
            try:
                ...
                if not (is_multi_channel and cell_crop.ndim == 3):
                    # Validate PIL image
    ...
    def _create_summary_visualization(self, original_img, mask, cell_crops, stats, output_dir, min_area, margin):
        ...
        try:
            ...
            print(f"âœ?Summary visualization successfully created at {output_path}")
            return output_path
        except Exception as e:
            print(f"â?Error creating summary visualization: {e}")
            import traceback
            traceback.print_exc()
            return None
...
if __name__ == "__main__":
    ...
    cd shape/tools/single_cell_cropper
    python tool.py
    """
