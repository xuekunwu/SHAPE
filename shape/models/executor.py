...
from shape.models.formatters import ToolCommand
from shape.models.utils import get_llm_safe_result
    ...
    execution = {{"error": "Failed to load metadata: " + str(e), "status": "failed"}}"""
        ...
        if tool_name == "Analysis_Visualizer_Tool" and previous_outputs:
            ...
            if has_adata_path or (has_analysis_type and has_cluster_key):
                logger.info(f"锟?Using special handling for Analysis_Visualizer_Tool with Cell_State_Analyzer_Tool results")
                ...
                if not adata_path:
                    logger.error(f"锟?adata_path is empty in Cell_State_Analyzer_Tool output. Cannot proceed with visualization.")
                    # Fall through to standard command generation
                else:
...
execution = tool.execute(
    ...
    figure_size=(10, 6),
                ...
                logger.warning(f"鈿狅笍 Analysis_Visualizer_Tool special handling skipped: missing required fields. "
        ...
        if tool_name == "Single_Cell_Cropper_Tool" and previous_outputs:
            ...
            if mask_path:
                ...
                # Use mask_path directly as-is (from Cell_Segmenter_Tool, already absolute)
                # Escape backslashes for Python string in command
                ...
                return ToolCommand(
                    ...
                    command=f"""execution = tool.execute(original_image="{actual_image_path}", nuclei_mask="{mask_path_str}", min_area=50, margin=25, query_cache_dir=r'{query_cache_dir_str}'{source_image_id_param}{group_param})"""
                )
            else:
    ...
    def execute_tool_command(self, tool_name: str, command: str) -> Any:
            ...
            try:
                ...
                with open(output_file, "w", encoding="utf-8") as f:
                    with redirect_stdout(f), redirect_stderr(f):
                        try:
                            # Use globals() to allow access to global namespace
        ...
        if tool_class is None:
            ...
            if tool_class is None:
                ...
                module_name = f"shape.tools.{tool_dir}.tool"
                try:
        ...
        try:
            ...
            # Using globals() in exec() to allow access to global namespace
