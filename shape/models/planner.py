...
from shape.models.formatters import QueryAnalysis, NextStep, MemoryVerification
from shape.models.tool_priority import ToolPriorityManager
from shape.utils import logger, ResponseParser
...
- For comparison: Analysis tool extracts features 鈫?Visualization tool displays results
"""
...
2. Determine the SINGLE main objective:
   - "How many cells/organoids" 鈫?Only counting needed (segmentation + counting, may visualize counts)
   - "What cell states" or "analyze cell states" 鈫?Full cell state analysis (preprocessing 鈫?segmentation 鈫?cropping 鈫?analysis 鈫?visualization)
   - "Compare" or "morphology" 鈫?Full pipeline (analysis tool extracts features, visualizer displays)
...
- Required Analysis Pipeline: Describe steps needed (e.g., "Image preprocessing (optional) 鈫?Segmentation 鈫?Count extraction")
...
**Pipeline Flow:**
1. Preprocessing (optional) 鈫?Image_Preprocessor_Tool
2. Segmentation 鈫?[Cell/Nuclei/Organoid_Segmenter_Tool] 鈫?produces masks
3. Cropping 鈫?Single_Cell_Cropper_Tool 鈫?REQUIRED: produces individual crop images (one per cell/organoid)
4. Analysis 鈫?Cell_State_Analyzer_Single_Tool (1 channel) | Cell_State_Analyzer_Multi_Tool (2+ channels)
...
5. Visualization 鈫?Analysis_Visualizer_Tool
...
**Query 鈫?Pipeline Mapping:**
- Counting: Segmentation 鈫?Count (visualize if needed)
- Morphology/Comparison/States: Full pipeline (Segmentation 鈫?Cropping 鈫?Analysis 鈫?Visualization)
...
- Workflow: Search tools (Pubmed_Search_Tool, Google_Search_Tool) 鈫?Generalist_Solution_Generator_Tool (synthesis with references)
...
<tool_name>: Exact tool name from available tools list"""
    ...
    def _format_tools_list(self, available_tools: List[str], used_tools: List[str]) -> str:
        ...
        lines = []
        for tool in available_tools:
            status = "鉁?Used" if tool in used_set else "Available"
...
- If query asks "how many cells" 鈫?Verify that cell count is available (from segmentation result) 鈫?STOP if count exists
- If query asks "what cell states" 鈫?Verify that cell state analysis (clustering + visualization) is complete 鈫?STOP if complete
- If query asks "compare" 鈫?Verify that comparison analysis is complete 鈫?STOP if complete
- DO NOT require information beyond what is explicitly asked in the query

CRITICAL: Tool Chain Completion Rules:
- If Analysis_Visualizer_Tool has been executed 鈫?The analysis pipeline is COMPLETE 鈫?STOP
- Analysis_Visualizer_Tool is the final step in the cell state analysis pipeline (Segmentation 鈫?Cropping 鈫?Cell_State_Analyzer 鈫?Analysis_Visualizer)
...
- If Analysis_Visualizer_Tool has been executed 鈫?STOP (pipeline complete)
