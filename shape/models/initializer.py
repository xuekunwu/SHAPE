...
class Initializer:
    ...
    def get_project_root(self):
        ...
        while current_dir != '/':
            if os.path.exists(os.path.join(current_dir, 'shape')):
                return os.path.join(current_dir, 'shape')
            current_dir = os.path.dirname(current_dir)
        raise Exception("Could not find project root")
        
    def class_name_to_dir(self, class_name: str) -> str:
        # 绫诲悕锛堝Fibroblast_Activation_Scorer_Tool锛夎浆涓虹洰褰曞悕锛坒ibroblast_activation_scorer锟?
    ...
    def dir_to_class_name(self, dir_name: str) -> str:
        # 鐩綍鍚嶈浆涓虹被鍚嶏紙濡俧ibroblast_activation_scorer -> Fibroblast_Activation_Scorer_Tool锟?
    ...
    def load_tools_and_get_metadata(self) -> Dict[str, Any]:
        ...
        self.toolbox_metadata = {}
        shape_dir = self.get_project_root()
        tools_dir = os.path.join(shape_dir, 'tools')
        print(f"SHAPE directory: {shape_dir}")
        print(f"Tools directory: {tools_dir}")
        sys.path.insert(0, shape_dir)
        sys.path.insert(0, os.path.dirname(shape_dir))
    ...
    def _set_up_tools(self) -> None:
        ...
        # 濡傛灉enabled_tools涓虹┖锛屽垯鑷姩鍙戠幇鎵€鏈夊伐锟?
